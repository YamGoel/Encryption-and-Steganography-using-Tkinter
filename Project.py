...
class NIS_Project:
    ...
    def main(self, root):
        ...
        roottitle = tk.Label(root, text="What do you want to do?")
        ...
        def new_encryption_frame(root):
            ...
            encryption_frame.geometry("850x400")

            enctitle = tk.Label(encryption_frame, text="What do you want to encrypt?")
            ...
            def new_text_frame(encryption_frame):
                ...
                text_frame.geometry("600x300")

                texttitle = tk.Label(text_frame, text="Enter text:")
                ...
                subText = tk.Button(text_frame, text="Submit", command=lambda: subText(text_frame), width=50, height=2,  bg='lightgreen', activebackground='green')
            ...
            def new_audio_frame(encryption_frame):
                ...
                def open_audio():
                    ...
                    filetypes = [('m4a', '*.m4a'),("mp3", "*.mp3"),('wav', '*.wav')]  
                    )
        ...
        def new_decryption_frame(root):
            ...
            dectitle = tk.Label(decryption_frame, text="Decryption")
            ...
            if not myStegFile:
                ...
                decryptImageButton = tk.Button(decryption_frame,  bg='lightgreen', activebackground='cyan', text="Decrypt Image", command=lambda: toImage(decryption_frame))
                decryptImageButton.pack(pady=5)

                def toImage(decryption_frame):
                    NIS_Project.chacha20_decrypt_image()
                    messagebox.showinfo("Success","Image has been Decrypted")
                    print("Image has been Decrypted")
                    decryption_frame.destroy()
                   

                decryptAudioButton = tk.Button(decryption_frame,  bg='lightgreen', activebackground='cyan', text="Decrypt Audio", command=lambda: toAudio(decryption_frame))
                decryptAudioButton.pack(pady=5)

                def toAudio(decryption_frame):
                    NIS_Project.chacha20_decrypt_audio()
                    messagebox.showinfo("Success","Audio has been Decrypted")
    ...
    def performSteganography(encrypted_data, data):
        ...
        steganography.geometry("500x500")
        ...
        if not myfile:
            ...
            img = ImageTk.PhotoImage(new_image)
            stegtitle= tk.Label(steganography,text='Create Stegano Object')
            stegtitle.config(font=('Helvetica',14,'bold'))
